...
* Az alapul szolgáló adatokkal kapcsolatos változások egyik vizualizációban sem jelennek meg. Szükséges *frissítése*, befejezéséig az adatokat fogja importálni.
* A jelentés (.pbix-fájl) **Power BI szolgáltatásban** való közzétételekor, egy adatkészlet jön létre, amely fel lesz töltve a Power BI szolgáltatásba.  Az importált adatok ebbe az adatkészletbe kerülnek. Ezt követően lehetősége például, hogy az adatok ütemezett frissítés beállításához, és importálja újra az adatokat minden nap. Az eredeti adatforrás helyétől függően szükség lehet egy Helyszíni adatátjáró konfigurálására.
...
* A jelentés közzététele után a **Power BI szolgáltatás**, újra fogja eredményezni az adatkészletet a Power BI szolgáltatásban, azonos importálása. Azonban ebbe az adatkészletbe *nem kerülnek adatok*.
...
A **DirectQuery** használata negatív következményekkel is járhat, amelyeket ez a szakasz ismertet. Ezen korlátozások némelyike kis mértékben különbözhet az itt leírtaktól, a pontos használt forrástól függően. Ez tüntetünk fel ahol pedig külön cikkek terjed ki a jelentősen különböző forrásokat.  
...
Hasonlóképpen az új jelentés szerkesztésének kell lekérdezéseket kell elküldeni az elérési utat az egyes lépések a kívánt végső Vizualizáció létrehozásához.
...
Egyes lekérdezésre négy perces időtúllépési idő vonatkozik a **Power BI szolgáltatás**, és ennél tovább tartó lekérdezések, amelyek sikertelen lesz. Ahogy korábban is hangsúlyoztuk, a DirectQueryt olyan adatforrásokhoz javasolt használni, amelyek közel interaktív lekérdezési teljesítménnyel bírnak, így ennek a korlátozásnak a célja is a túl hosszú végrehajtási időkből eredő problémák megelőzése.

### <a name="other-implications"></a>Egyéb következmények
Néhány egyéb általános következményei használatával **DirectQuery** az alábbiak szerint:

* **Az adatok módosítása esetén a legújabb adatok megjelenítéséhez frissítés szükséges:** A gyorsítótárak használata miatt nem biztosítható, hogy a vizualizáció mindig a legfrissebb adatokat jelenítse meg. Mutathatják például az elmúlt egy nap tranzakcióit. Majd egy szeletelő módosítása, mert, előfordulhat, hogy frissül, és a tranzakciók megjeleníti az elmúlt két napban, beleértve az egyes legutóbbi, újonnan érkezett tranzakciót. Ha a szeletelőt visszaállítjuk az eredeti értékére, akkor újra a korábban gyorsítótárazott értéket fogja mutatni, amely nem tartalmazza az imént látott, újonnan érkezett tranzakciót.
...
* **Mértékszűrők:** A mértékeket (vagy mértékek összegeit) tartalmazó vizualizációk a mértékeken belüli szűrőket is tartalmazhatnak. Például az alábbi vizualizáció az értékesített mennyiségeket mutatja kategóriák szerint, de csak a 20 milliós értékesítés feletti kategóriákat tartalmazza.
  ...
  * Ezután a második lekérdezés lekéri a vizualizációhoz szükséges adatokat, beleértve a kategóriákat, amelyek megfeleltek a WHERE utasításban szereplő feltételeknek.  
  ...
  Ez általában hajt végre működik, ha több száz vagy ezer kategória, mint ebben a példában. Ha a kategóriák száma sokkal nagyobb (és a valóban, a lekérdezés meghiúsul, ha egynél több kategória felel meg a feltételnek, a korábban említett egymillió soros határértéket miatt) a teljesítmény csökken.
* **Legjobb N szűrők:** Megadhatók fejlettebb szűrők, amelyek egy adott mérték által rangsorolt első (vagy utolsó) N számú értéket szűrik ki, például csak az első 10 kategóriát a fenti vizualizációban. Ehhez szintén két lekérdezést kell elküldeni az alapul szolgáló forrás felé. Az első lekérdezésre azonban az alapul szolgáló forrás az összes kategóriát visszaadja, majd a legjobb N értéket a visszaadott eredmények alapján választja ki a rendszer. Az érintett oszlop számosságától függően vezethet a teljesítménnyel kapcsolatos problémákat (vagy sikertelen lekérdezéseket az 1-m sor határérték miatt).
* **Medián:** Általában a rendszer minden összesítést (Összeg, Eltérők darabszáma stb.) elküld az alapul szolgáló forrásnak. Ez azonban nem igaz a Középérték esetében, mivel ezt az összesítést az alapul szolgáló források általában nem támogatják. Ezekben az esetekben a Power BI részletadatokat kérdez le az alapul szolgáló forrástól, és ezek visszaadott eredményei alapján számítja ki a középértéket. Ez akkor ésszerű, ha a középérték eredmények viszonylag kis számú ki kell, de a teljesítménnyel kapcsolatos problémákat (vagy sikertelen lekérdezéseket az 1-m sor határérték miatt) történik, ha a számosság nagy.  Például egy megye lakosságának mediánját jól ki lehet számolni, de az értékesítési árak mediánját már nem biztos.
* **Speciális szövegszűrők („tartalmaz” és hasonlók):** Egy szöveges oszlop szűrésekor a speciális szűrők között megtalálhatók a „tartalmaz”, „kezdődik” és hasonló szűrők. Ezek teljesítménycsökkenést okozhatnak bizonyos adatforrások esetében. Különösen az alapértelmezett "tartalmaz" szűrőt nem lehet használja, ha mire szükség ("" vagy "nem") pontosan egyeznie. Bár az eredmények ugyanazok lehetnek, a konkrét adatoktól függően a teljesítményben komoly különbségek mutatkozhatnak az indexek használata miatt.
...
A DirectQuery megnyitja az egyidejű kapcsolatok maximális száma alapértelmezett tíz. Ezt módosíthatja az aktuális fájlhoz a **Power BI Desktop** **Fájl > Lehetőségek és beállítások > Beállítások** területén, a bal oldali panel **Aktuális fájl** szakaszában, a **DirectQuery** lehetőség kiválasztásával. 
...
További lényeges oszlopok a következők:

* **TextData:** Az esemény szöveges részletezése. A „Lekérdezés kezdete/vége” eseményeknél ez jelöli a DAX-lekérdezést. A "DirectQuery kezdete/vége" eseményeknél ez lesz az alapul szolgáló forrásnak elküldött SQL-lekérdezést. A jelenleg kiválasztott esemény szöveges adatai az alsó régióban is megjelennek.
...
* Nyissa meg az **SQL Server Profiler** eszközt, és vizsgálja meg a nyomkövetési adatokat a fentebb leírt módon. Ne feledje, hogy a nyomkövetési fájl a **Power BI Desktop** bezárásakor törlődik. Ezenkívül további műveleteket a Power BI Desktopban nem azonnal jelenik meg – a nyomkövetési fájl kell kell zárni és újra megnyitja az új események megtekintéséhez.
