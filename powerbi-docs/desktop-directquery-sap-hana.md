...
* **Az SAP HANA többdimenziós forrásként való kezelése (alapértelmezett):**  Ebben az esetben a rendszer hasonlóan viselkedik ahhoz, amikor a Power BI más többdimenziós forrásokhoz, például az SAP Business Warehouse-hoz vagy az Analysis Serviceshez kapcsolódik. SAP HANA-t, ezzel a beállítással, egyetlen elemzési vagy számítási nézet van kiválasztva, összes való csatlakozáskor hierarchiák és attribútumok az adott nézet érhetők el a mezők listájában. A vizualizációk létrehozásakor az adatok összesítése mindig az SAP HANA-ból lesz lekérdezve. Ez az ajánlott módszer, és egyúttal az alapértelmezett beállítás is az SAP HANA-t használó új DirectQuery-jelentéseknél.

* **Az SAP HANA relációs forrásként való kezelése:** Ebben az esetben a Power BI relációs forrásként kezeli az SAP HANA-t. Ez nagyobb rugalmasságot. Gondot kell fordítani ezt a módszert használja, győződjön meg arról, hogy a mértékek elvárt módon legyenek összesítve, és teljesítménybeli problémák elkerülése érdekében.

Egy globális beállítással, amely kiválasztásával értéke határozza meg a kapcsolati módszer **fájl > lehetőségek és beállítások** , majd **beállítások > DirectQuery**, majd pedig  **SAP HANA relációs forrásként való kezelése**, ahogy az alábbi képen látható. 

![](media/desktop-directquery-sap-hana/directquery-sap-hana_01a.png)

Ha a HANA relációs forrásként való kezelését választotta, az az SAP HANA-ra vonatkozó DirectQueryt használó minden létrehozott *új* kapcsolatot is befolyásol. Az aktuális jelentésben már meglévő SAP HANA-kapcsolatokra azonban nincs hatással, ahogyan a többi megnyitott jelentés kapcsolatait sem befolyásolja. Ezért ha ez a beállítás nincs engedélyezve, akkor ha az **Adatok beolvasása** lehetőséggel új SAP HANA-kapcsolatot ad hozzá, akkor ez a kapcsolat az SAP HANA-t többdimenziós forrásként kezeli majd. Azonban ha egy másik jelentés meg van nyitva, amely is SAP Hana-hoz kapcsolódik, akkor a jelentés továbbra is működik, amely be lett állítva a beállítást alkalmazza *létrehozták időben*, ami azt jelenti, hogy azok a jelentések az SAP Hana-val, is 2018 Februárja előtt létrehozott továbbra is az SAP HANA relációs forrásként való kezelése. 

A két megközelítést jelent eltérő viselkedést, és az nem lehetséges egy meglévő jelentést a másikba. 
...
Ha SAP Hana-hoz mint többdimenziós forráshoz kapcsolódik, a következő szempontokat kell figyelembe venni:

* Az **Adatok beolvasása navigátorban** egyetlen SAP HANA-nézet választható ki. Egyedi mértékek vagy attribútumok nem választhatók ki. A kapcsolódás idején nincs definiálva lekérdezés, ellentétben az adatimportálással vagy a DirectQuery használatával, amikor az SAP HANA-t relációs forrásként kezeli a rendszer. Ez azt is jelenti, hogy már nem használandó közvetlenül az SAP HANA SQL-lekérdezést a kapcsolódási módszerrel.
...
* Az SAP HANA-ban meghatározható, hogy az attribútumok címkeként egy másik attribútumot használjanak. Ha például **termék** (az 1,2,3 értékeket, és így tovább) használhat **ProductName** (az értékei bicikli, ing, kesztyű stb) címkeként. Ebben az esetben a mezőlistában egyetlen mezőként a **Product** jelenik meg, amelynek az értékei Bicikli, Ing, Kesztyű stb., de amelynek rendezési alapja és az egyedisége az 1,2,3 kulcsértékek alapján lesz meghatározva. Emellett létre lesz hozva a rejtett **Product.Key** oszlop is, amely lehetővé teszi, hogy szükség esetén hozzá lehessen férni az alapul szolgáló értékekhez is. 

Az alapul szolgáló SAP HANA-nézetben definiált minden változó megjelenik a kapcsolódáskor, és a szükséges értékek így megadhatók. Ezeket az értékeket később módosítható kiválasztásával **lekérdezések szerkesztése** a menüszalagról, majd **paraméterek kezelése** jelenik meg a legördülő menüből. 
...
Ha a **adatok lekérése** élmény, a választott tartoztak **ProductID** és a **AveragePrice** mértékeket, akkor az egy lekérdezést a nézetben, kér, amely határozza összesített adatok (az előző példában az egyszerűség kedvéért egy ál-SQL szerepel, amely nem felel meg tökéletesen az SAP HANA SQL szintaxisának). Ekkor a vizualizáción belül meghatározott minden további összesítés tovább összesíti az ilyen lekérdezések eredményét. Az SQL Server esetében fentebb leírtakhoz hasonlóan ez szintén érvényes az importálásra és a DirectQuery használatára is. A DirectQuery esetben származó lekérdezést **adatok lekérése** vagy **Lekérdezésszerkesztő** SAP Hana-hoz küldött egyetlen lekérdezés részkiválasztásaként használja, és így nem ténylegesen az összes adat volna olvasni az előzetes eset a további összesítés.  
...
* Az **Adatok lekérése** utasításban vagy a **Lekérdezésszerkesztőben** csak a szükséges oszlopokat kell kiválasztani a szükséges adatok lekéréséhez, hiszen az eredmény lekérdezés lesz, amelynek az SAP HANA-hoz továbbítható ésszerű lekérdezésnek kell lennie. Például ha több tucat oszlopot választott ki, hogy lehet rájuk későbbi vizualizációkban, a gondolat a majd még a DirectQuery esetében egy egyszerű vizualizációt kiválasztva azt jelenti, hogy a részkiválasztásban használt összesítő lekérdezés tartalmazni fogja a tucat oszlopot, amely általában mutattak.
  
Lássunk erre egy példát. Az alábbi példában öt oszlop van kiválasztva (a **CalendarQuarter**, a **Color**, a **LastName**, a **ProductLine** és a **SalesOrderNumber**) az **Adatok lekérése** párbeszédablakban, illetve az *OrderQuantity* mérték, ez pedig azt jelenti, hogy később egy a Min OrderQuantity (Minimális rendelési mennyiség) mértéket tartalmazó egyszerű vizualizációt kiválasztva az eredmény a következő SQL-lekérdezést küldi az SAP HANA-hoz. Az árnyékolt szakasz a részkiválasztást jelöli, amely az **Adatok lekérése** / **Lekérdezésszerkesztőből** származó lekérdezést tartalmazza. Ha ez a részkiválasztás magas számosságú eredményt ad, majd az SAP HANA-teljesítményt valószínűleg gyenge is.  
...
* **Kapcsolódás SSL használatával** - importálás segítségével csatlakoztathatja, és SSL-lel többdimenziós vételi nem lehet csatlakozni a relációs összekötő SSL használatára konfigurált SAP HANA-példányok.
